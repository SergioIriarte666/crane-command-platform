-- SOLUCIÓN DEFINITIVA: Arreglo de Trigger + Recuperación de Notificaciones Perdidas
...
BEGIN
    ...
    -- Insertar notificación (incluso si user_id es NULL, siempre que tengamos teléfono para WhatsApp)
...
-- 4. Re-conectar el Trigger a la tabla services (INSERT y UPDATE)
...
-- 5. RECUPERACIÓN DE DATOS: Generar notificaciones para servicios recientes sin notificar
INSERT INTO public.notifications (
    tenant_id, user_id, title, message, type, channel, status, metadata
)
SELECT 
...
WHERE s.updated_at > (now() - interval '24 hours') -- Solo servicios de las últimas 24h
AND s.operator_id IS NOT NULL
AND NOT EXISTS (
    SELECT 1 FROM notifications n 
    WHERE (n.metadata->>'service_id')::uuid = s.id
);
