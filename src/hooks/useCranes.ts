...
export function useCranes() {
  ...
  const createCrane = useMutation({
    mutationFn: async (crane: Omit<CraneInsert, 'tenant_id'>) => {
      ...
      // Check plan limits
      if (tenant.max_cranes !== null && tenant.max_cranes !== undefined) {
        ...
        if (count !== null && count >= tenant.max_cranes) {
          throw new Error(`Has alcanzado el límite de grúas de tu plan (${tenant.max_cranes}). Contacta a soporte para aumentar tu plan.`);
        }
      }
