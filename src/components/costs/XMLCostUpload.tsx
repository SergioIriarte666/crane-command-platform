  ...
  const handleUploadCosts = async () => {
    ...
    try {
      ...
      for (let i = 0; i < total; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map(async (xmlCost, batchIndex) => {
          const originalIndex = i + batchIndex;
          const categoryId = categoryMappings[originalIndex] || getDefaultCategoryId(xmlCost.categoria);
          ...
          const costCode = `XML-${Date.now()}-${originalIndex}`;
        ...
        setUploadProgress(Math.round((Math.min(i + BATCH_SIZE, total) / total) * 100));
      }
